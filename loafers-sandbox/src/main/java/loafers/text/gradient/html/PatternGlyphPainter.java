...
public class PatternGlyphPainter extends GlyphView.GlyphPainter {
    ...
    public PatternGlyphPainter(String text, GlyphView v, JTextPane textPane, Pattern pattern) {
        if (v != null) {
            this.text = text;
            this.font = v.getFont();
            init(v);
    ...
    public GlyphView.GlyphPainter getPainter(GlyphView v, int p0, int p1) {
        try {
            ...
            return new PatternGlyphPainter(localText, v, textPane, pattern);
        } catch (BadLocationException e) {
            e.printStackTrace();
        }
        return null;
    ...
    float getTabbedTextWidth(View view, String txtStr, float x, TabExpander e, int startOffset, int[] justificationData) {
        float nextX = x;
    ...
    float drawTabbedText(View view, String txtStr, float x, float y, Graphics g, TabExpander e, int startOffset,
        ...
        for (int i = txtOffset; i < n; i++) {
            if (txt[i] == '\t'
                    || ((spaceAddon != 0 || i <= spaceAddonLeftoverEnd) && (txt[i] == ' ')
                            && startJustifiableContent <= i && i <= endJustifiableContent)) {
                if (flushLen > 0) {
                    drawGlyphVector(graphics, glyphVector, x, y);

                    //corrected position
                    nextX += (glyphVector.getGlyphPosition(flushIndex + flushLen).getX() - glyphVector
                            .getGlyphPosition(flushIndex).getX());
                    flushLen = 0;
                }
                flushIndex = i + 1;
                if (txt[i] == '\t') {
                ...
                if (flushLen > 0) {
                    ...
                    //corrected
                    nextX += (glyphVector.getGlyphPosition(flushLen).getX() - glyphVector.getGlyphPosition(flushIndex)
                            .getX());
                    flushLen = 0;
                }
                flushIndex = i + 1;
    ...
    int getTabbedTextOffset(View view, String s, int x0, int x, TabExpander e, int startOffset, boolean round,
            int[] justificationData) {
        if (x0 >= x) {
            // x before x0, return.
            return 0;
